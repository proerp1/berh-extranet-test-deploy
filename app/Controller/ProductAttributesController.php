...
class ProductAttributesController extends AppController
{
    ...
    public function add($productId)
    {
        ...
        if ($this->request->is(['post', 'put'])) {
            ...
            if ($this->ProductAttribute->save($this->request->data)) {
                $this->Flash->set(__('O atributo foi salvo com sucesso'), ['params' => ['class' => "alert alert-success"]]);
                $this->redirect(['action' => 'index', $productId]);
            } else {
                $this->Flash->set(__('O atributo não pode ser salvo, Por favor tente de novo.'), ['params' => ['class' => "alert alert-danger"]]);
            }
        }
    ...
    public function delete($id)
    {
        ...
        if ($this->ProductAttribute->save($data)) {
            $this->Flash->set(__('O atributo foi excluido com sucesso'), ['params' => ['class' => "alert alert-success"]]);
