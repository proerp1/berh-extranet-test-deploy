...
class PlanoContasController extends AppController
{
            ...
            if ($this->PlanoConta->validates()) {
                ...
                if (empty($planocontas)) {
                    if ($this->PlanoConta->save($this->request->data)) {
                        $this->Session->setFlash(__('O plano de conta foi salvo com sucesso'), 'default', ['class' => "alert alert-success"]);
                        $this->redirect(['action' => 'edit/'.$this->PlanoConta->id]);
                    } else {
                        $this->Session->setFlash(__('O plano de conta não pode ser salvo, Por favor tente de novo.'), 'default', ['class' => "alert alert-danger"]);
                    }
                } else {
                    $this->Session->setFlash(__('O plano de conta não pode ser salvo, este número de identificação já existe.'), 'default', ['class' => "alert alert-danger"]);
                }
            } else {
                $this->Session->setFlash(__('O plano de conta não pode ser salvo, Por favor tente de novo.'), 'default', ['class' => "alert alert-danger"]);
            }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if (empty($planocontas)) {
                if ($this->PlanoConta->save($this->request->data)) {
                    $this->Session->setFlash(__('O plano de conta foi alterado com sucesso'), 'default', ['class' => "alert alert-success"]);
                    $this->redirect(['action' => 'edit/'.$this->PlanoConta->id]);
                } else {
                    $this->Session->setFlash(__('O plano de conta não pode ser alterado, Por favor tente de novo.'), 'default', ['class' => "alert alert-danger"]);
                }
            } else {
                $this->Session->setFlash(__('O plano de conta não pode ser salvo, este número de identificação já existe.'), 'default', ['class' => "alert alert-danger"]);
            }
    ...
    public function delete($id)
    {
        ...
        if ($this->PlanoConta->save($this->request->data)) {
            $this->Session->setFlash(__('O plano de conta foi excluido com sucesso'), 'default', ['class' => "alert alert-success"]);
