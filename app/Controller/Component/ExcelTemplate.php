    ...
    $activeWorksheet->getStyle('E')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
            ...
            for ($i = 0; $i < count($dados); $i++) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('J' . $indx, ($dados[$i]["Outcome"]["supplier_id"] ?? '') . ' - ' . ($dados[$i]["Supplier"]["nome_fantasia"] ?? ''))
                    ->setCellValue('K' . $indx, 'Fornecedor')
...
public function getFluxo($objPHPExcel, $dados, $conta)
{
    ...
    for ($i = 0; $i < count($dados); $i++) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('K' . $indx, isset($dados[$i][0]['nome']) ? $dados[$i][0]['nome'] : '');
    ...
    public function getFaq($objPHPExcel, $dados)
    {
        ...
        // Cabeçalhos (ordem fixa)
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "ID Fornecedor"); $col++;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Última Modificação"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "ID FAQs"); $col++;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Sistema Destino");
        ...
        foreach ($dados as $dado) {
            ...
            if ($temTodos) {
                // Única linha indicando "Todos os fornecedores" com ID Fornecedor em branco
                $col = 'A';
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, ''); $col++;                  // ID Fornecedor
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, 'Todos os fornecedores'); $col++; // Fornecedor
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $created); $col++;            // Data de Criação
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $modified); $col++;           // Última Modificação
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $faqId); $col++;              // ID FAQs
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $pergunta); $col++;           // Pergunta
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $resposta); $col++;           // Resposta
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $categoria); $col++;          // Categoria
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $sistema);                    // Sistema Destino
                $linha++;
            } else {
                // Uma linha por fornecedor específico
                foreach ($relacionamentos as $rel) {
                    $supplierId = $rel['supplier_id'] ?? null;
                    $fornecedor = $rel['Supplier']['nome_fantasia'] ?? '';

                    if (empty($supplierId) || empty($fornecedor)) {
                        continue;
                    }

                    $col = 'A';
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $supplierId); $col++;     // ID Fornecedor
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $fornecedor); $col++;     // Fornecedor
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $created); $col++;        // Data de Criação
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $modified); $col++;       // Última Modificação
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $faqId); $col++;          // ID FAQs
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.$linha, $pergunta); $col++;       // Pergunta
    ...
    public function getRelatorioComprasSimples($objPHPExcel, $dados)
    {
        $col = 'A';
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "CNPJ CLIENTE"); $col++;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Dias Úteis"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Id(Código Operadora)"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Número do Cartão (Vale Transporte)"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "VlUnit"); $col++;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Pedido"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "GE-CNPJ"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Liberação do crédito"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Período Inicio"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Período Fim"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Compra Operadora"); $col++;
        ...
        foreach ($dados as $key => $dado) {
            ...
            $quantity = $dado['OrderItem']['manual_quantity'] != 0 ? 
                        $dado['OrderItem']['manual_quantity'] : 
                        $dado['CustomerUserItinerary']['quantity'];

            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Customer']['documento']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['name']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['cpf']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['rg']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['emissor_rg']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['data_nascimento']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['nome_mae']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['OrderItem']['working_days']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Supplier']['code']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUserItinerary']['card_number']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['OrderItem']['valor_unit']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['OrderItem']['working_days']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $quantity); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['cep']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['endereco']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['numero']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['complemento']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['bairro']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['cidade']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado[0]['estado']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['CustomerUser']['sexo']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Order']['id']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['EconomicGroups']['document']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Order']['credit_release_date']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Order']['order_period_from']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado['Order']['order_period_to']); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), number_format(($dado['OrderItem']['subtotal_not_formated'] - $dado['OrderItem']['saldo_not_formated']), 2, ',', '.')); $col++;
    ...
    public function getAtendimento($objPHPExcel, $data)
{
    ...
    public function getPedidosBeneficiariosPIX($objPHPExcel, $dados) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Chave PIX ou Copia e Cola"); $col++; // 1
    ...
    public function getCanais($objPHPExcel, $dados)
    {
        $col = 'A';
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Status"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Tipo Canal"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Tipo Pessoa"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Conta Bancária"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Razão Social"); $col++;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Cidade"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Estado"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Telefone"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Celular"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Celular 2"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Site"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Vencimento"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Valor Recebido por cliente"); $col++;
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col.'1', "Observação"); $col++;
        ...
        foreach ($dados as $key => $dado) {
            ...
            $tipo = $tipos[$dado['Resale']['tipo']];
            $tipos_pessoa = ['1' => 'Física', '2' => 'Jurídica'];
            $tipo_pessoa = $tipos_pessoa[$dado['Resale']['tipo']];

            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Status"]["name"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $tipo); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $tipo_pessoa); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["BankAccount"]["name"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["razao_social"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["nome_fantasia"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["email"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["cnpj"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["inscricao_estadual"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["inscricao_municipal"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["cep"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["endereco"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["numero"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["complemento"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["bairro"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["cidade"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["estado"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["telefone"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["celular"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["celular2"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["site"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Vencimento"]["name"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["valor_recebido_cliente"]); $col++;
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($col . ($key+2), $dado["Resale"]["observation"]); $col++;
        }
    }
}
