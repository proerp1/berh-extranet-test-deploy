<?php $url_novo = $this->base.'/order_documents/add/'.$id; ?>
<?php echo $this->element('../Orders/_abas'); ?>

<div class="card mb-5 mb-xl-8">
    <form action="<?php echo $this->Html->url(['controller' => 'order_documents', 'action' => 'index', $id]); ?>" role="form" id="busca" autocomplete="off">
                    ...
                    <?php if ($data) { ?>
                        <?php for ($i = 0; $i < count($data); ++$i) { ?>
                            <tr>
                                ...
                                <td class="fw-bold fs-7 ps-4"><?php echo date('d/m/Y H:i:s', strtotime($data[$i]['OrderDocument']['created'])); ?></td>

                                <td class="fw-bold fs-7 ps-4">
                                    <a href="<?php echo $this->base.'/order_documents/edit/'.$id.'/'.$data[$i]['OrderDocument']['id']; ?>" class="btn btn-info btn-sm">
                                        Editar
                                    </a>
                                    <a href="javascript:" onclick="verConfirm('<?php echo $this->base.'/order_documents/delete/'.$id.'/'.$data[$i]['OrderDocument']['id']; ?>');" rel="tooltip" title="Excluir" class="btn btn-danger btn-sm">
                                        Excluir
                                    </a>
                                </td>
                            </tr>
