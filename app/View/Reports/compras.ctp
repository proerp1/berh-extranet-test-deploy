    ...
    $(document).ready(function() {
        ...
        $('#alterar_confirm').on('click', function(e) {
            ...
            if ($(".check_all").is(':checked')) {
                ...
                formData.append('curr_aba', urlParams.get('aba') || '');
                formData.append('curr_q', urlParams.get('q') || '');
                formData.append('curr_sup', urlParams.get('sup') || '');
                formData.append('curr_c', urlParams.get('c') || '');
                formData.append('curr_de', urlParams.get('de') || '');
                formData.append('curr_para', urlParams.get('para') || '');
                formData.append('curr_num', urlParams.get('num') || '');
                formData.append('curr_first_order', urlParams.get('first_order') || '');

                const curr_st = Array.from(document.querySelectorAll('#kt-toolbar-filter #st option:checked')).map(el => el.value);
                const curr_stp = Array.from(document.querySelectorAll('#kt-toolbar-filter #stp option:checked')).map(el => el.value);
                const curr_stpg = Array.from(document.querySelectorAll('#kt-toolbar-filter #stpg option:checked')).map(el => el.value);
                const curr_bt = Array.from(document.querySelectorAll('#kt-toolbar-filter #bt option:checked')).map(el => el.value);

                formData.append('curr_st', JSON.stringify(curr_st));
                formData.append('curr_stp', JSON.stringify(curr_stp));
                formData.append('curr_stpg', JSON.stringify(curr_stpg));
                formData.append('curr_bt', JSON.stringify(curr_bt));
