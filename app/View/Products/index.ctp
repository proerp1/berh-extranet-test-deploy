<?php $url_novo = $this->base."/products/add/"; ?>
<div class="card mb-5 mb-xl-8">
    <form action="<?php echo $this->Html->url(array( "controller" => "products", "action" => "index")); ?>" role="form" id="busca" autocomplete="off">
                        ...
                        <div class="px-7 py-5">
                            <div class="mb-10">
                                ...
                                <select class="form-select form-select-solid fw-bolder" data-kt-select2="true" data-placeholder="Selecione" data-allow-clear="true" name="tipo" id="tipo">
                                    <option></option>
                                    <?php
                                        foreach (['1' => 'SERASA', '2' => 'BeRH', '3' => 'Me Proteja', '4' => 'String'] as $tipoId => $tipo) {
                                            $selected = "";
                                            if (isset($_GET["tipo"]) && $tipoId == $_GET["tipo"]) {
                                                $selected = "selected";
                                            }
                                            echo '<option value="'.$tipoId.'" '.$selected.'>'.$tipo.'</option>';
                                        }
                                    ?>
                                </select>
                            </div>
                    ...
                    <?php if ($data) { ?>
                        <?php for ($i=0; $i < count($data); $i++) { ?>
                            <tr>
                                ...
                                <td class="fw-bold fs-7 ps-4"><?php echo $data[$i]["Product"]["valor"]; ?></td>
                                <td class="fw-bold fs-7 ps-4">
                                    <a href="<?php echo $this->base.'/products/edit/'.$data[$i]["Product"]["id"]; ?>" class="btn btn-info btn-sm">
                                        Editar
                                    </a>
                                    <a href="javascript:" onclick="verConfirm('<?php echo $this->base.'/products/delete/'.$data[$i]["Product"]["id"]; ?>');" rel="tooltip" title="Excluir" class="btn btn-danger btn-sm">
                                        Excluir
                                    </a>
                                </td>
                            </tr>
