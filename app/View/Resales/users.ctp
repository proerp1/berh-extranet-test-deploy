<?php $url_novo = $this->base."/resales/add_user/".$id.'/?'.(isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '');  ?>
<?php
...
<div class="card mb-5 mb-xl-8">
    <form action="<?php echo $this->Html->url(array( "controller" => "resales", "action" => "users", $id)).'/?'.(isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : ''); ?>" role="form" id="busca" autocomplete="off">
                    ...
                    <?php if ($data) { ?>
                        <?php for ($i=0; $i < count($data); $i++) { ?>
                            <tr>
                                ...
                                <td class="fw-bold fs-7 ps-4">
                                    <a href="<?php echo $this->base.'/resales/edit_user/'.$id.'/'.$data[$i]["CustomerUser"]["id"].'/?'.(isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : ''); ?>" class="btn btn-info btn-sm">
                                        Editar
                                    </a>
                                    <?php if ($data[$i]["CustomerUser"]["main_user"] != 1): ?>
                                        <a href="javascript:" onclick="verConfirm('<?php echo $this->base.'/resales/delete_user/'.$id.'/'.$data[$i]["CustomerUser"]["id"].'/?'.(isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : ''); ?>');" rel="tooltip" title="Excluir" class="btn btn-danger btn-sm">
                                            Excluir
                                        </a>
                                    <?php endif ?>
                                </td>
