<?php $url_novo = $this->base."/billing_sales/add/";  ?>
<div class="card mb-5 mb-xl-8">
    <form action="<?php echo $this->Html->url(array( "controller" => "billing_sales", "action" => "index")); ?>/" role="form" id="busca" autocomplete="off">
                    ...
                    <?php if ($dados) { ?>
                        <?php for ($i=0; $i < count($dados); $i++) { ?>
                            <tr>
                                ...
                                <td class="fw-bold fs-7 ps-4"><?php echo date('m/Y', strtotime($dados[$i]["BillingSale"]["mes_pagamento"])) ?></td>
                                <td class="fw-bold fs-7 ps-4"><?php echo $dados[$i]["User"]["name"] ?></td>
                                <td class="fw-bold fs-7 ps-4"> 
                                    <a href="<?php echo $this->base.'/billing_sales/revenda/'.$dados[$i]["BillingSale"]["id"]; ?>" class="btn btn-info btn-sm">
                                        Ver
                                    </a> 
                                    <a href="javascript:" onclick="verConfirm('<?php echo $this->base.'/billing_sales/delete/'.$dados[$i]["BillingSale"]["id"]; ?>');" rel="tooltip" title="Excluir" class="btn btn-danger btn-sm">
                                        Excluir
                                    </a>
                                </td>
                            </tr>
